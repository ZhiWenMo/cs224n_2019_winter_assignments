...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.combined_output_projection = nn.Linear(
            3*self.hidden_size, hidden_size, bias=False)
        self.target_vocab_projection = nn.Linear(
            self.hidden_size, len(self.vocab.tgt))
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # YOUR CODE HERE (~ 8 Lines)
        X = self.model_embeddings.target(source_padded)
        X = nn.utils.rnn.pack_padded_sequence(
            X, source_lengths)

        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens = nn.utils.rnn.pad_packed_sequence(
            enc_hiddens, batch_first=True)[0]

        cat_last_hidden = torch.cat(
            (last_hidden[0, :, :], last_hidden[1, :, :]), dim=1)
        init_decoder_hidden = self.h_projection(cat_last_hidden)

        cat_last_cell = torch.cat(
